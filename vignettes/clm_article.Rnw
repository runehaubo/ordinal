...
% the explanatory variables, is normally distributed. This cannot be the
...
% Methods for continuous data are in practice often applied to ordinal data
% and if there are many response categories and the response does not pile
% up in the end categories, such methods can also be useful -- in particular in the initial stages of a data analysis.
...
function.
% , $F$ is the CDF of a distribution in the location-scale
% family of distributions. Common choices of $F$ are the CDFs of the
% logistic, normal, and Gumbel distributions.
%
...
% \emph{I should probably refer to $\bm\alpha$ (or similar symbol) for the joint parameter vector so that I don't have to distinguish between $\theta$ and $\beta$.}
...
% \begin{table}[t!]
%   \begin{center}
%   \caption{Summary of various link functions.\label{tab:linkFunctions}}
%   \footnotesize
...
\begin{table}[t!]
  ...
  (\url{http://en.wikipedia.org/wiki/Gumbel_distribution}). \\
  $^c$: the Cauchy distribution is a $t$-distribution with one df
  \caption{Summary of the five standard link functions.\label{tab:linkFunctions}}
\end{table}
...
% Random-effects can also be included,
% but we defer the discussion of CLMs with random-effects, the cumulative link
% mixed models (CLMMs) to section XX.
...
% Structured thresholds can be combined with nominal effects which leads to...
...
% In cases where a parameter is undefined the likelihood function will be completely flat in that direction which causes the Hessian to have one eigenvalue at zero.
...
%with the link functions considered in Table~\ref{tab:linkFunctions}%, except for the cauchit link,
...
% When the log-likelihood is not strictly concave it means that the Hessian matrix can become singular or negative definite. For this reason the NR algorithm regularizes the Hessian matrix whenever it is not positive definite by adding an amount corresponding to $1 + $ the smallest negative eigenvalue to the diagonal before computing the Newton step. There is no guarantee that this heuristic leads to a step in a direction of greater likelihood so more and more is added to the diagonal until the step is in a favorable direction.
...
%
\begin{description}
  \item[-3] Not all thresholds are increasing. This is only possible with nominal effects and constitues and invalid fit.
...
%
% \emph{Using the standard treatment contrast coding means that thresholds refer to the first level of \code{temp} (\code{"cold"}) and \code{contact} (\code{"no"})}
%
% \emph{It is helpful to think of $\bm x_i^\top \bm\beta$ as modelling a shift of the thresholds $\{\theta\}$ and we may denote by $\tilde{\theta}_{ij} = \theta_j \bm x_i^\top \bm\beta$ the $j$'th threshold for the $i$'th observation.%
% }
...
\begin{figure}
  ...
  \caption{Illustration of the effect of temperature in the standard cumulative link model in Equation~\eqref{eq:CLM} for the wine data in Table~\ref{tab:wineData} through a latent variable interpretation.\label{fig:standard_clm}}
\end{figure}
...
The cumulative link model in \eqref{eq:CLM} assumes that the
...
% $\{\theta_{ij} = \theta_j + \theta_{2j} (\mathtt{contact}_i)\}$
...
%  ~~\hat\beta_2(\mathtt{yes}) - \hat\beta_2(\mathtt{no}) = 1.53, \\
...
% parameters have \code{.(Intercept)} appended their names indicating
% that these are the estimates of $\theta_j$ corresponding to the first level of contact (\code{"no"}).
% The following five threshold parameters
% have \code{.contactyes} appended their name indicating that these
% parameters are differences between the threshold parameters at the two
% levels of contact (\code{yes" - "no"}).
%
% As can be seen from the output,
% the effect of \code{contact} is almost constant across thresholds
% and around $-1.5$ corresponding to the estimate of contact ($\beta_2$) from \code{fm1} on
% except for the change of sign.

\begin{figure}
  \centering
  \includegraphics[width=6cm]{./static_figs/fig-figNom2}
  \caption{Illustration of nominal effects leading to different sets of thresholds being applied for each level of \code{contact} in a latent variable interpretation, cf., Equation~\eqref{eq:CLM_nominal}.\label{fig:clm_nominal}}
\end{figure}
...
%
% Latent variable motivation - location-scale model - scale effects in an CLM - one way to handle non-proportional odds.
%
% \code{nominal_test}, \code{scale_test}, sign and interpretation of nominal effects, the \code{Theta} component.
...
\begin{figure}
  ...
  \caption{Illustration of scale effects leading to different scales of the latent variable, cf., Equation~\eqref{eq:CLM_scale_wine}.\label{fig:clm_scale}}
\end{figure}
...
% any further here.
%
% The effects of \code{contact} and \code{temp} can be illustrated
% with
...
\end{equation*}
that is, the value of $\tilde\pi_i$, cf., Equation~\eqref{eq:clm-log-likelihood} evaluated at the ML estimates $\hat{\bm\psi}$. These are the values returned by the \code{fitted} and \code{fitted.values} extractor methods and stored in the \code{fitted.values} component of the model fit.

The values of $\pi_{ij}$ (cf., Equation~\eqref{eq:multinom_pmf}) evaluated at the ML estimates of the parameters (i.e., $\hat\pi_{ij}$) can also be thought of as fitted values for the multinomially distributed variable $\bm Y_i^*$. These values can be obtained from the model fit by use of the \code{predict} method:
<<>>=
...
Cumulative link models is a very rich model class in between the perhaps the two most important model classes in statistics; linear models and logistic regression models. With the greater flexibility relative to binary logistic regression comes the option to check assumptions such as the partial proportional odds assumption. Non-linear structures such as scale effects arise naturally in a latent variable interpretation. In addition to nominal effects and the non-linear scale effects, the ordered nature of the thresholds gives rise to computational challanges which we have described and addressed in the \pkg{ordinal} package.

In addition to computational challenges, practical data analysis with CLMs can also be challanging. In our experience a top-down approach in which a ``full'' model is fitted and gradually simplified is often problematic, not only because this easily leads to unidentiable models but also because there are many different ways in which models can be reduced or expanded. A more pragmatic approach is often preferred; understanding the data through plots, tables, and even linear models can aid in finding a suitable intermediate ordinal starting model.

Attempts to inditify a ``correct'' model will also often lead to frustrations; the greater the model framework, the greater the risk that there are multiple models which fit the data (almost) equally well. It is well known statistical wisdom that with enough data many goodness of fit tests become sensitive to even minor deviations of little practical relevance. This is particularly true for tests of partial proportional odds; in the author's experience almost all CLMs on real data show some evidence of non-proportional odds for one or more variables while the most useful representation of the data is a model that simply assumes proportional odds.
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% The author which to thank Professor Per Bruun Brockhoff many valuable discussions.
